...
function Cart() {
  ...
  async function increaseQuantity(id) {
      ...
      const token = localStorage.getItem('userToken');
            const {data} = await axios.patch(`${import.meta.env.VITE_URL}/cart/incraseQuantity`,
      {"productId":id},
      {
          headers:{
  ...
  async function decreaseQuantity(id) {
    ...
    try{
    const token = localStorage.getItem('userToken');
          const {data} = await axios.patch(`${import.meta.env.VITE_URL}/cart/decraseQuantity`,
    {"productId":id},
    {
        headers:{
          Authorization:`Tariq__${token}`
...
async function removeItem(id) {
  ...
  try{
  const token = localStorage.getItem('userToken');
        const {data} = await axios.patch(`${import.meta.env.VITE_URL}/cart/removeItem`,
  {"productId":id},
  {
      headers:{
        Authorization:`Tariq__${token}`
...
async function removeAll() {
  ...
  try{
  const token = localStorage.getItem('userToken');
        const {data} = await axios.patch(`${import.meta.env.VITE_URL}/cart/clear`,{'':''},
  {
      headers:{
        Authorization:`Tariq__${token}`
      }
...
// end remove all
  ...
  return (
    <div className="container text-center mt-5 border-top">
            ...
            <div className="d-flex flex-column mb-5 gap-2 justify-content-center align-items-center">
                <h2 className='  mt-3 fs-1 fw-bold'>Your Cart</h2>
                <p className='text-secondary text-capitalize'>Fill your cart and enjoy purchasing what you need.</p>
                {cartProducts.length>0?<button className={style.removeAll}  onClick={()=>{
                  removeAll();
                  }} disabled={loading||cartProducts.length==0?'disabled':''}>{loading?'wait...':'clear all'}</button>:''}
            </div>
 ...
 <div className="d-flex justify-content-center mt-5">
  ...
  ): (!error?( Products.length>0?(<div className={style.cartBoxes}>
    {cartProducts.map((product) => (
      <div className={style.cartBox} key={product._id}>
        <div className={style.boxStart}>
          <img src={product.details.mainImage.secure_url} alt="" />
          <div className={style.boxStartInformation}>
            <h3>{product.details.name}</h3>
            {product.details.discount>0?
               <div className={style.withDicount}>
              <span className={style.finalPrice}>{product.details.finalPrice}$</span>
              <span className={style.startPrice} >{product.details.price}$</span></div> :
               <span className={style.price} >{product.details.price}$</span>}
        ...
        <div className={style.boxEnd }>
          ...
          <div className={style.quantity}>
            <button  onClick={()=>{
              decreaseQuantity(product.details.id);
              
              }} disabled={loading||product.quantity<=1?'disabled':''}>-</button>
            <span>{product.quantity}</span>
            <button onClick={()=>{
              increaseQuantity(product.details.id);
              
              }} disabled={loading?'disabled':''}>+</button>
          </div>
          <button className={style.delete} onClick={()=>{removeItem(product.details.id)}}>
  ...
  shopping to fill your Cart</p>)):
  <p>error to Fetch cart {error}</p>)
   }
